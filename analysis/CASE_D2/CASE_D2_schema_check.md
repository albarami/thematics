# CASE_D2 Schema Check

## Gate 2 purpose
This file locks the structured-artifact schema for `CASE_D2` before coding begins, using the current `CASE_D1` artifact set as the operational baseline.

## Gate 2 status
- **Status**: Locked for the active Day 2 build
...
### CASE_D2 question-by-theme matrix CSV
...
### CASE_D2 prominence / salience CSV
Authoritative Day 1 header:
```csv
...
## Gate 2 decision
Until a CSV mirror is explicitly created for Day 1 and adopted as the baseline, the Day 2 working codebook will follow the current operational Day 1 structure:
...
This is an explicit documented exception to the QA spec wording that mentions a codebook CSV. The live workspace baseline is markdown, and Day 2 will not invent a different codebook format mid-case.
...
## 4. Existing CASE_D2 Gate 1 schema checks already satisfied

The following Day 2 Gate 1 artifacts already match the active baseline structure:
- `CASE_D2_source_register.csv`
- `CASE_D2_participant_register.csv`
- `CASE_D2_question_map.md`
- `CASE_D2_preparation_checklist.md`

`CASE_D2_participant_register.csv` already includes the locked `speaker_type` field and uses only the allowed values.

---

## 5. Pre-coding requirements for the remaining CASE_D2 artifacts

Before the Day 2 coding layer is treated as valid:
...
`CASE_D2` may proceed to the coding/evidence layer under the schema lock above.

The only documented schema exception is the working codebook format: the current workspace baseline is markdown rather than CSV, and the Day 2 build will preserve that baseline unless the project explicitly migrates both cases to a new canonical codebook format.
