...
CANDIDATE_THEMES = ROOT / "CASE_D4_candidate_themes.md"
EXCERPT_BANK = ROOT / "CASE_D4_excerpt_bank.csv"
FINAL_REPORT = ROOT / "CASE_D4_final_report.md"
FINAL_THEMES = ROOT / "CASE_D4_final_themes.md"
MATRIX_PROMINENCE_CHECK = ROOT / "CASE_D4_matrix_prominence_check.md"
PARTICIPANT_REGISTER = ROOT / "CASE_D4_participant_register.csv"
PARTICIPANT_SUMMARY = ROOT / "CASE_D4_participant_summary.csv"
PARTICIPANT_WORKBOOK = ROOT / "CASE_D4_participant_workbook.xlsx"
PREPARATION_CHECKLIST = ROOT / "CASE_D4_preparation_checklist.md"
PROMINENCE_SALIENCE = ROOT / "CASE_D4_prominence_salience.csv"
QUESTION_EVIDENCE = ROOT / "CASE_D4_question_evidence_table.csv"
QUESTION_THEME_MATRIX = ROOT / "CASE_D4_question_theme_matrix.csv"
SUMMARY_TABLES = ROOT / "CASE_D4_summary_tables.md"
THEME_EVIDENCE_WORKBOOK = ROOT / "CASE_D4_theme_evidence_workbook.xlsx"
THEME_INTEGRITY = ROOT / "CASE_D4_theme_integrity_report.md"
THEME_SUMMARY = ROOT / "CASE_D4_theme_summary_table.csv"
BOUNDARY_MEMO = ROOT / "CASE_D4_boundary_memo.md"
FAMILIARISATION_MEMO = ROOT / "CASE_D4_familiarisation_memo.md"
LANGUAGE_MEMO = ROOT / "CASE_D4_language_memo.md"
SOURCE_SENSITIVITY_MEMO = ROOT / "CASE_D4_source_sensitivity_memo.md"
REPORT_INTEGRITY = ROOT / "CASE_D4_report_integrity_check.md"
FINAL_CROSSCHECK = ROOT / "CASE_D4_final_crosscheck_report.md"
ANONYMIZED_PARTICIPANT_WORKBOOK = ROOT / "participant_summary_anonymized.xlsx"
OUTWARD_PARTICIPANT_REGISTER = OUTWARD_DIR / "CASE_D4_participant_register.csv"
...
NAME_REPLACEMENTS = {
    "Diana Hassan": "moderator",
    "Diana (moderator)": "moderator",
    "Diana": "moderator",
    "Dr Abdellatif Moderator": "moderator",
    "Abdellatif": "moderator",
    "أستاذة مريم": "الأخصائية",
    "أ.مريم": "participant",
    "مريم": "participant",
    "دكتورة فيروز": "participant",
    "د.فيروز": "participant",
    "فيروز": "participant",
    "دكتور علاء": "doctor",
    "د.علاء الدين الكيلاني": "participant",
    "علاء الدين الكيلاني": "participant",
    "أستاذة عبلة": "الأستاذة",
    "عبلة": "participant",
}

LIMITATION_TERMS = (
    "`HWEL9AR` granularity and conversational over-segmentation",
    "`HWEL9AR` question-boundary ambiguity",
    "`HWEL10AR` merged late section",
    "`D4_P25` dominance",
    "transcript-derived evidence layer only",
)
...
CROSS_CASE_TERMS = ("CASE_D1", "CASE_D2", "CASE_D3", "CASE_D5")
...
def _report_ids(report_text: str) -> list[str]:
    return sorted(set(re.findall(r"D4_S\d{4}", report_text)))
...
def _build_report_integrity(
    ...
    no_moderator_codes = not any(re.search(r"D4_M\d{2}", line) for line in quote_lines)
    ...
    support_labels_explicit = all(
        term in report_text
        for term in (
            "Coded transcript support",
            "Close-reading support",
            "Contextual note support",
            "Auxiliary recommendation support",
        )
    )
    ...
    return "\n".join(
        [
            "# CASE_D4 Report Integrity Check",
            "",
            ...
            f"| No cross-case claims | {'PASS' if no_cross_case_terms else 'FAIL'} — CASE_D4 report text {'does not reference other cases' if no_cross_case_terms else 'references other cases'} |",
            ...
            f"| Known Day 4 limitations remain explicit | {'PASS' if limitations_explicit else 'FAIL'} — required limitation statements {'are' if limitations_explicit else 'are not'} visible across the final report layer |",
...
def _build_final_crosscheck(
    ...
    theme_names_locked = all(
        theme_name in report_text
        and theme_name in FINAL_THEMES.read_text(encoding="utf-8")
        and theme_name in summary_text
        for theme_name in theme_names
    )
    salience_locked = all(
        row["salience"] in summary_text
        and row["salience"].replace("_", " ") in report_text.lower().replace("-", " ")
        for row in prominence_rows
    )
    report_ids_resolve = all(evidence_id in excerpt_lookup for evidence_id in report_ids)
    moderator_exclusion = not any(re.search(r"D4_M\d{2}", line) for line in quote_lines)
    ...
    return "\n".join(
        [
            "# CASE_D4 Final Cross-Check Report",
            "",
            "## Case: CASE_D4 — Day 4 (Elderly)",
            ...
            f"| 6 | Note-taker material is truthfully handled | {'PASS' if note_rows == 0 else 'FAIL'} — {note_rows} note rows operationalized; {unclear_rows} `unclear` transcript rows retained in the excerpt bank |",
            ...
            f"| 8 | Day 4 source limitations remain explicit | {'PASS' if limitations_explicit else 'FAIL'} |",
...
def main() -> None:
    """Build the Day 4 outward/internal packages and Gate 6/7 check files.

    Returns:
        None. Writes sanitized outward-facing artifacts, the internal identity key,
        Gate 6/7 verification files, and the final Day 4 package folders.
    """
    ...
    print(f"Name scan hits: {final_scan_hits}")
