...
RAW_DIR = ROOT / "Day4_Elderly"
TRANSCRIPTS_DIR = RAW_DIR / "Audio Recordings _Transcripts"
NOTES_DIR = RAW_DIR / "NoteTakers_Notes"
OUT_DIR = ROOT / "analysis" / "CASE_D4" / "_working"
TRANSCRIPTS_JSON = OUT_DIR / "d4_transcripts_extracted.json"
NOTES_JSON = OUT_DIR / "d4_notes_extracted.json"
...
def extract_docx_text(path: Path) -> dict[str, object]:
    ...
    Returns:
        A structured record containing filename, paragraph count, character count,
        paragraph list, and concatenated full text.
    """
...
def dump_folder(folder: Path, output: Path) -> None:
    """Extract all `.docx` files in one folder to a JSON file.
    ...
    Returns:
        None.
    """
    records = [extract_docx_text(path) for path in sorted(folder.glob("*.docx"))]
