...
WORKING_DIR = ROOT / "analysis" / "CASE_D4" / "_working"
TRANSCRIPTS_JSON = WORKING_DIR / "d4_transcripts_extracted.json"
NOTES_JSON = WORKING_DIR / "d4_notes_extracted.json"
REPORT_PATH = WORKING_DIR / "case_d4_gate1_diagnostics.md"

label_patterns = [
    re.compile(r"^Speaker\s+\d+:", re.IGNORECASE),
    re.compile(r"^Moderator:", re.IGNORECASE),
    re.compile(r"^Question\s+\d+:", re.IGNORECASE),
    re.compile(r"^Part\s+\d+:", re.IGNORECASE),
    re.compile(r"^المحاور:?$"),
    re.compile(r"^مدير(?:ة)? الجلسة:?$"),
    re.compile(r"^الميسر(?:ة)?:?$"),
    re.compile(r"^مشارك(?:\s+آخر)?(?:\s+\([^)]*\))?:"),
    re.compile(r"^المشارك(?:\s+الأول|\s+الثاني|\s+الثالث|\s+الآخر)?:"),
...
lines: list[str] = ["# CASE_D4 Gate 1 Diagnostics", ""]
...
for record in transcripts:
    ...
    if candidate_labels:
        for item in candidate_labels[:50]:
            lines.append(f"  - {item}")
    else:
        lines.append("  - none detected")
    lines.append("- Question cues:")
    if question_cues:
        for item in question_cues[:30]:
            lines.append(f"  - {item}")
    else:
        lines.append("  - none detected")
    lines.append("")
